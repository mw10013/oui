...
- Oui "copy-paste" ui components are located in `registry/default/ui/oui-*.tsx`.
- Shadcn ui components are in `registry/default/ui` and their filenames do not have the `oui-` prefix.
- Oui demo components are located in `registry/default/components/oui-*-demo.tsx`.
...
- `refs/react-spectrum` contains a read-only local reference copy of the Adobe React Spectrum monorepo, downloaded via `pnpm refs:react-spectrum`. It is ignored by Git and used for LLM scanning/reference only. The react-aria-components (RAC) package code is located in `refs/react-spectrum/packages/react-aria-components/`.
- `refs/react-day-picker` contains a read-only local reference copy of the React Day Picker repo, downloaded via `pnpm refs:react-day-picker`. It is ignored by Git and used for LLM scanning/reference only. The react-day-picker package code is located in `refs/react-day-picker/`.
